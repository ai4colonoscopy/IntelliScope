...
from colongpt.train.colongpt_trainer import ColongptTrainer

from colongpt import conversation as conversation_lib
from colongpt.model import *
from colongpt.util.data_utils import make_supervised_data_module, DataArguments
...
def train():
    ...
    if model_args.model_type == 'phi-1.5' or model_args.model_type == 'phi-2':
        model = ColongptPhiForCausalLM.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=training_args.cache_dir,
    ...
    if training_args.lora_enable:
        ...
        if not target_modules:
            print("No linear modules found in the model.")
        else:
    ...
    data_module = make_supervised_data_module(tokenizer=tokenizer,
                                              data_args=data_args)
    trainer = ColongptTrainer(model=model,
                           tokenizer=tokenizer,
                           args=training_args,
                           **data_module)
